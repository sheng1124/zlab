...
def detect(opt):
    ...
    half = device.type != 'cpu' and device.type != 'mps' and not save_txt_tidl  # half precision only supported on CUDA
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Apply NMS
        if device.type == 'mps':
            pred = non_max_suppression(pred.to('cpu'), opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
        else:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                #img torch.Size([1, 3, 640, 1088])
                #im0 (1080, 1920, 3)
                #img torch.Size([1, 3, 384, 640])
                #im0 (1080, 1920, 3)
...
# python detect.py --weights yolov7-w6-pose.pt --conf 0.25 --source 0 --device mps --kpt-label
