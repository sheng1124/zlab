...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        #attempt_download(w)
        print(f'weights = {w} load to {map_location}')
        if map_location == torch.device("mps"):
            ckpt = torch.load(w, map_location = torch.device("cpu"))
            ckpt_mps = ckpt['ema' if ckpt.get('ema') else 'model'].float().fuse().eval().to('mps')
            print(type(ckpt_mps))
            model.append(ckpt_mps)
        else:
            print('???')
